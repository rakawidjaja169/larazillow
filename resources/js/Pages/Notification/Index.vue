<template>
  <div class="w-full">
    <h1 class="text-3xl mb-4 w-full">Your Notifications</h1>

    <section v-if="notifications.data.length" class="text-gray-700 dark:text-gray-400 w-full">
      <div v-for="notification in notifications.data" :key="notification.id" class="border-b border-gray-200 dark:border-gray-800 py-4 flex justify-between items-center">
        <div>
          <span v-if="notification.type === 'App\\Notifications\\OfferMade'">
            Solution: "{{ notification.data.solution }}" for
            <Link
              :href="route('user.listing.show', { listing: notification.data.listing_id })" 
